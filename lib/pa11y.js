...
async function runPa11yTest(url, options, state) {
	...
	page.on('response', response => {
		if (response.url().indexOf(url) !== -1) {
			options.log.debug('RESPONSE:', response.url());
		}
	});

	// Navigate to the URL we're going to test
	options.log.debug(`Opening URL in Headless Chrome: ${url}`);
	...
	options.log.debug(`Injecting HTML CodeSniffer: ${url}`);
	await page.evaluate(htmlCodeSnifferJavaScript);
	options.log.debug(`Injecting Pa11y: ${url}`);
	await page.evaluate(pa11yRunnerJavaScript);

	// Launch the test runner!
	options.log.debug(`Running Pa11y on the page: ${url}`);
	...
	try {
		...
		options.log.debug(`> ERROR when running Pa11y on ${url}: ${pa11yError}`);
		throw pa11yError;
	}
	/* eslint-enable no-shadow */

	options.log.debug(`Document title: "${results.documentTitle}" ${url}`);
